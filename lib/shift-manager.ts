  export const DRAFT_KEY = (userId: string) => `currentShiftDraft_${userId}`;
  export const SHIFT_ID_KEY = (userId: string) => `currentShiftId_${userId}`;
  ...
  export interface Shift {
    ...
    netTotal: number
    incidents?: string
    status: "pending" | "reviewed" | "active" | "rejected"
    driverEmail: string
    reviewedBy?: string
    reviewedAt?: string
    reviewNotes?: string
  ...
  export class ShiftManager {
    ...
    static async saveShift(shift: Shift): Promise<boolean> {
      try {
        ...
        const currentShiftData = typeof window !== 'undefined'
          ? JSON.parse(localStorage.getItem(DRAFT_KEY(userId)) || '{}')
          : {};
        const shiftData = {
          driver_id: userId,
        ...
        if (data?.id) {
          localStorage.setItem(SHIFT_ID_KEY(userId), data.id); 
    ...
    static async updateShift(updatedShift: Shift): Promise<boolean> {
      const { supabase } = await import('./supabase-client');
      // Forzamos siempre status = 'pending' al actualizar
      const { error } = await supabase
        .from('driver_shifts')
        .update({
          ...updatedShift,
          status: 'pending',
          bonus_earned: updatedShift.bonusEarned
        })
        .eq('id', updatedShift.id);
        if (!error && typeof window !== 'undefined') {
          // ← NUEVO: borrar borrador tras enviar la actualización
